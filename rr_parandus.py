...
suurim_protsent = 0.0
for i in range(mitu):
    protsent_kokku = ((yksdoosN[i] + yksdoosM[i]) - (t2ielikultN[i] + t2ielikultM[i])) / (naised[i] + mehed[i]) * 100
    if protsent_kokku > suurim_protsent:
        suurim_protsent = protsent_kokku
        indeks = i

poolikult_vaktsineeritud = (yksdoosM[indeks] + yksdoosN[indeks]) - (t2ielikultM[indeks] + t2ielikultN[indeks])
print("Kõige suurema pooliku vaktsineeritusega inimeste osakaal on: ", haldusyksused[indeks], liigid[indeks], "Selles kohas on poolikult vaktsineeritud", poolikult_vaktsineeritud, "inimest ja nende osakaal elanikest on ", "{0:0.2f}".format(suurim_protsent), "protsenti.")
